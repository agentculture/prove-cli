"""``prove-cli learn`` — the learnability affordance.
...
from prove.cli._output import emit_result

_TEXT = """\
prove-cli — a clonable template for AgentCulture mesh agents.
...
Commands
--------
  prove-cli whoami             Identity from culture.yaml.
  prove-cli learn              This self-teaching prompt.
  prove-cli explain <path>...  Markdown docs for any noun/verb path.
  prove-cli overview           Descriptive snapshot of the agent.
  prove-cli doctor             Check the agent-identity invariants.
  prove-cli cli overview       Describe the CLI surface itself.
...
def _as_json_payload() -> dict[str, object]:
    return {
        ...
        "json_support": True,
        "explain_pointer": "prove-cli explain <path>",
    }
