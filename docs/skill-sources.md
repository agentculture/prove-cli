...
prove-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. prove-cli
...
#   - consumer-identifying prose: `guildmaster` → `prove-cli` (NOT
...
If a re-sync would lose a prove-cli adaptation, lift the change
...
#     → `which prove-cli vendors from guildmaster`
