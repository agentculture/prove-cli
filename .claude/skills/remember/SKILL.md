...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope prove-cli
...
scope:** the default personal scope is **private** (`--scope prove-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `prove-cli`). Private records are served only to a recall in the same
